...
export function normaliseTag(jitPlugins, modifiedPlugins, p) {
        ...
        try {
            ...
            if (name.includes(':')) {
                ...
                if (!Object.values(results).every(Boolean)) {
                    ux.warn(`This plugin may not work as expected because the prepare script did not produce all the expected files.`);
                }
            }
            else {
                // npm
    ...
    async link(p, { install }) {
        ...
        if (install) {
            if (await fileExists(join(c.root, 'yarn.lock'))) {
                ...
                ux.warn(`The ${ux.colorize('dim', 'plugins link')} command will no longer auto install dependencies in future versions. Please run ${ux.colorize('dim', 'yarn')} manually in the plugin directory before linking.`);
                await yarn.install([], {
                    cwd: c.root,
                    logLevel: this.logLevel,
                });
