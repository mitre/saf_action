...
exports.FULL_NIST_HIERARCHY = exports.NistRevision = exports.NistControl = exports.DEFAULT_CANONIZATION_CONFIG = void 0;
...
const NIST_FAMILY_RE = /^(?:A[CPRTU]|C[AMP]|D[IM]|I[APR]|M[AP]|P[ELMS]|RA|S[ACEI]|TR|U[LM])$/;
// Limit length of children to avoid potential DoS on malicious NIST Control strings
const NIST_CONTROL_RE = /^(A[CPRTU]|C[AMP]|D[IM]|I[APR]|M[AP]|P[ELMS]|RA|S[ACEI]|TR|U[LM])-(\d+)(.{0,60})$/;
const SPEC_SPLITTER = /[\s\(\)\.]+/; // Includes all whitespace, periods, and parenthesis
const REV_RE = /^rev[\s_.]+(\d+)$/i; // Matches Rev_5 etc
exports.DEFAULT_CANONIZATION_CONFIG = {
    max_specifiers: 5,
    pad_zeros: false,
    allow_letters: true,
    add_spaces: true,
...
function default_partial_config(c) {
    return { ...exports.DEFAULT_CANONIZATION_CONFIG, ...c };
}
...
class NistControl {
     ...
     */
    subSpecifiers; // Guaranteed to be of length at least one on a "real" control
...
function _generate_full_nist_hierarchy() {
    ...
    raw_nist_1.ALL_NIST_CONTROL_NUMBERS.forEach((n) => {
        const asControl = parse_nist(n); // We know there are no revs in our file
        if (!asControl) {
