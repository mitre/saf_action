 ...
 */
class GenerateUpdateControls extends base_command_1.BaseCommand {
    ...
    static flags = {
        xccdfXmlFile: core_1.Flags.string({
            char: 'X',
            description: '\u001B[31m(required [-X or -U])\u001B[34m The XCCDF XML file containing the new guidance - in the form of .xml file',
            exclusive: ['xccdfUrl'],
        }),
        xccdfUrl: core_1.Flags.url({
            char: 'U',
            description: '\u001B[31m(required [-X or -U])\u001B[34m The URL pointing to the XCCDF file containing the new guidance (DISA STIG downloads)',
    ...
    async run() {
        ...
        (0, cli_helper_1.addToProcessLogData)('==================== Update Controls for Delta Process =====================');
        ...
        if (flags.xccdfXmlFile) {
            xccdfXmlFile = (0, global_1.basename)(flags.xccdfXmlFile);
            logger.info(`Verifying that the XCCDF file is valid: ${xccdfXmlFile}...`);
            if (isXccdfFile(flags.xccdfXmlFile, logger)) {
                xccdfContent = fs_1.default.readFileSync(flags.xccdfXmlFile, 'utf8');
            }
            else {
                return;
            }
        ...
        const shortControlsDir = path_1.default.sep + (0, global_1.basename)(path_1.default.dirname(flags.controlsDir))
            + path_1.default.sep + (0, global_1.basename)(flags.controlsDir);
        // -------------------------------------------------------------------------
        ...
        if (flags.inspecJsonFile) {
            logger.info(`  Using execution/profile summary file: ${(0, global_1.basename)(flags.inspecJsonFile)}`);
            try {
                if (fs_1.default.lstatSync(flags.inspecJsonFile).isFile()) {
                    ...
                    throw new Error(`Input execution/profile JSON file not found: ${(0, global_1.basename)(flags.inspecJsonFile)}.\n`
                        + 'Run the --help command to more information on expected input files.');
                }
            }
            catch (error) {
                if (error instanceof Error) {
                    const errorCode = error.code; // Safe access to `code`
                    if (errorCode === 'ENOENT') {
            ...
            try {
                ...
                const inspecJsonFile = (0, child_process_1.execSync)(`cinc-auditor json '${profileDir}'`, { encoding: 'utf8', maxBuffer: 50 * 1024 * 1024 });
        ...
        const xccdfProfile = (0, inspec_objects_1.processXCCDF)(xccdfContent, false, idType);
        ...
        for (const control of xccdfProfile.controls) {
            let controlId;
            const newControlId = (0, global_1.basename)(control.id); // Ensure there aren't any leading or trailing slashes
            ...
            controlId = (0, global_1.basename)(controlId || '');
        ...
        for (const file of files) {
            ...
            if (fileExt === ext) {
                ...
                const currentFileFullPath = path_1.default.join(controlsDir, (0, global_1.basename)(file));
                const currentControlNumber = (0, global_1.basename)(path_1.default.parse(file).name);
                ...
                if (currentControlNumber === xccdfNewControlNumber) {
                    ...
                    if (baselineYControls.has(currentControlNumber)) {
                        logger.debug('  \u001B[32mUpdating Control metadata with XCCDF content\u001B[0m');
                    ...
                    if (baselineYControls.has(currentControlNumber)) {
                        ...
                        logger.debug('  \u001B[31mKeeping Control metadata content - Updating Control name\u001B[0m');
                        notInProfileJSON++;
        ...
        logger.on('finish', () => {
            (0, cli_helper_1.addToProcessLogData)('\nProcess Statistics ===========================================================================\n');
            if (flags.formatControls) {
                (0, cli_helper_1.printGreen)('╔════════════════════════════════════════════════════════════════════════════════╗');
                (0, cli_helper_1.printGreen)('║ Controls were formatted the same way `generate delta` will update the controls ║');
                (0, cli_helper_1.printGreen)('╚════════════════════════════════════════════════════════════════════════════════╝');
            }
            else {
                (0, cli_helper_1.printRed)('╔════════════════════════════════════════════════════════════════════════════════════╗');
                (0, cli_helper_1.printRed)('║ Controls were NOT formatted the same way `generate delta` will update the controls ║');
                (0, cli_helper_1.printRed)('╚════════════════════════════════════════════════════════════════════════════════════╝');
            }
            (0, cli_helper_1.printYellowGreen)('\n     Total skipped files (no mapping to new control Id):', `${skipped.toString().padStart(4)}`);
            (0, cli_helper_1.printYellowGreen)('Total processed files (found mapping to new control Id): ', `${processed.toString().padStart(3)}`);
            (0, cli_helper_1.printYellowGreen)('\n    Total controls with correct identification: ', `${isCorrectControl.toString().padStart(3)}`);
            (0, cli_helper_1.printYellowGreen)('Total new controls found in the XCCDF guidance: ', `${newControls.toString().padStart(3)}`);
            ...
            if (notInProfileJSON > 0) {
                (0, cli_helper_1.printBoldRedGreen)('\nTotal skipped metadata update (not found in Input execution/profile JSON file): ', `${notInProfileJSON.toString().padStart(3)}`);
                (0, cli_helper_1.printBoldRedGreen)('Control(s) skipped metadata update: ', `${skipMetadataUpdate.toString()}`);
            }
...
function isXccdfFile(xccdfXmlFile, logger) {
    let isXccdf = true;
    try {
        ...
        if (error instanceof Error) {
            ...
            if (errorCode === 'ENOENT') {
                logger.error(`ERROR: No entity found for: ${xccdfXmlFile}. Run the --help command for more information on expected input files.`);
            }
            logger.error(`ERROR: Unable to process the XCCDF XML file ${xccdfXmlFile} because: ${error.message}`);
        }
        else {
            logger.error(`ERROR: An unexpected error occurred: ${(0, global_1.getErrorMessage)(error)}`);
        }
        isXccdf = false;
    }
    return isXccdf;
