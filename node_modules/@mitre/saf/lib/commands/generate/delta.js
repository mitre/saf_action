...
const base_command_1 = require("../../utils/oclif/base_command");
const cli_helper_1 = require("../../utils/oclif/cli_helper");
...
*/
class GenerateDelta extends base_command_1.BaseCommand {
    ...
    async run() {
        ...
        thisLogger.warn(colors_1.default.green('╚══════════════════════════════════════════════════════════════════════════════════════════════════════════════╝'));
        (0, cli_helper_1.addToProcessLogData)('==================== Delta Process =====================');
        (0, cli_helper_1.addToProcessLogData)(`Date: ${new Date().toISOString()}`);
        if (flags.interactive) {
            ...
            (0, cli_helper_1.addToProcessLogData)('Process Flags ===========================================');
            for (const [key, value] of Object.entries(flags)) {
                (0, cli_helper_1.addToProcessLogData)(`${key}=${value instanceof URL ? value.toString() : String(value)}`);
            }
        }
        else {
            return;
        }
        (0, cli_helper_1.addToProcessLogData)('\n');
        ...
        try {
            if (runMapControls && controlsDir) {
                ...
                (0, cli_helper_1.addToProcessLogData)('Mapping controls (using fuzzy logic - lower value = best match) from the old profile to the new profile\n');
                ...
                (0, cli_helper_1.printCyan)('Updating Controls ===========================================================================');
                ...
                for (const [key, value] of Object.entries(controls)) {
                    const sourceShortControlFile = path_1.default.join(shortProfileDir, `${value}.rb`);
                    const mappedShortControlFile = path_1.default.join(shortMappedDir, `${value}.rb`);
                    const sourceControlFile = path_1.default.join(controlsDir, `${value}.rb`);
                    const mappedControlFile = path_1.default.join(mappedDir, `${value}.rb`);
                    (0, cli_helper_1.printYellowGreen)('Mapping (From --> To): ', `${value} --> ${key}`);
                    let lines;
                    if (fs_1.default.existsSync(sourceControlFile)) {
                        lines = fs_1.default.readFileSync(sourceControlFile, 'utf8').split('\n');
                    }
                    else {
                        (0, cli_helper_1.printBgRedRed)('    File not found at:', ` ${sourceControlFile}\n`);
                        (0, cli_helper_1.printRed)('╔═══════════════════════════════════════════════════════════════════════════════╗');
                        (0, cli_helper_1.printRed)('║ Make sure the appropriate Input execution/profile JSON file is being used (-J)║');
                        (0, cli_helper_1.printRed)('╚═══════════════════════════════════════════════════════════════════════════════╝');
                        return;
                    }
                    ...
                    if (value === key) {
                        ...
                        (0, cli_helper_1.printYellowGreen)('   Control is Current: ', `${sourceShortControlFile}`);
                        // Saved processed control to the 'mapped_controls' directory
                        (0, cli_helper_1.printYellowGreen)('    Processed control: ', `${mappedShortControlFile}\n`);
                        fs_1.default.writeFileSync(mappedControlFile, lines.join('\n'));
                    }
                    else {
                        (0, cli_helper_1.printYellowGreen)('   Processing control: ', `${sourceShortControlFile}`);
                        ...
                        if (controlLineIndex === -1) {
                            (0, cli_helper_1.printBgRedRed)('    Control not found:', ` ${sourceControlFile}\n`);
                        }
                        else {
                            ...
                            (0, cli_helper_1.printYellowGreen)('  Mapped control file: ', `${sourceShortControlFile} to reference ID ${key}`);
                            (0, cli_helper_1.printYellowBgGreen)('     New control name: ', `${key}.rb\n`);
                        }
                    }
                }
        ...
        if (existingProfile && xccdfContent) {
            ...
            if (updatedResult) {
                ...
                (0, cli_helper_1.addToProcessLogData)('Update Results ===========================================================================\n');
                (0, cli_helper_1.addToProcessLogData)(updatedResult.markdown);
                await sleep(2000).then(() => (0, cli_helper_1.printGreen)('\nDelta Process completed successfully\n'));
                (0, cli_helper_1.saveProcessLogData)();
            }
            else {
    ...
    async mapControls(oldProfile, newProfile) {
        ...
        */
        ...
        (0, cli_helper_1.printCyan)('Mapping Process ===========================================================================');
        ...
        for (const newControl of newControls) {
            ...
            if (newControl.title) {
                ...
                if ((0, lodash_1.isEmpty)(result)) {
                    (0, cli_helper_1.printYellowGreen)('     New XCCDF Control:', ` ${newControlId}`);
                    (0, cli_helper_1.printBgYellow)('* No Mapping Provided *\n');
                    GenerateDelta.newXccdfControl++;
                    continue;
                }
                (0, cli_helper_1.printYellowBgGreen)('Processing New Control: ', `${newControlId}`);
                (0, cli_helper_1.printYellowBgGreen)('     New Control Title: ', `${this.updateTitle(newControl.title)}`);
                if (result[0]?.score && result[0].score < 0.3) { // skipcq: JS-W1044
                    if (controlIdToScoreMap.has(result[0].item.id)) {
                        ...
                        if (result[0].score < score) {
                            ...
                            (0, cli_helper_1.printBgMagentaRed)('     Old Control Title:', ` ${this.updateTitle(result[0].item.title)}`);
                            (0, cli_helper_1.printBgMagentaRed)('       Duplicate Match:', ` ${result[0].item.id} --> ${newControlId}`);
                            (0, cli_helper_1.printBgMagentaRed)('        Matching Score:', ` ${result[0].score}\n`);
                    ...
                    if (typeof newControlId === 'string'
                        ...
                        (0, cli_helper_1.printYellowGreen)('     Old Control Title: ', `${this.updateTitle(result[0].item.title)}`);
                        ...
                        if (result[0].score > 0.1) {
                            ...
                            (0, cli_helper_1.printBgRed)('** Potential Mismatch **');
                            GenerateDelta.posMisMatch++;
                        }
                        else {
                            GenerateDelta.match++;
                        }
                        (0, cli_helper_1.printYellowGreen)('  Best Match Candidate: ', `${result[0].item.id} --> ${newControlId}`);
                        (0, cli_helper_1.printYellowGreen)('        Matching Score: ', `${result[0].score}\n`);
                    ...
                    (0, cli_helper_1.printBgRedRed)('     Old Control Title:', ` ${this.updateTitle(result[0].item.title)}`);
                    (0, cli_helper_1.printBgRedRed)('    No Match Found for:', ` ${result[0].item.id} --> ${newControlId}`);
                    (0, cli_helper_1.printBgRedRed)('        Matching Score:', ` ${result[0].score} \n`);
                    GenerateDelta.noMatch++;
                }
            }
        }
        (0, cli_helper_1.printCyan)('Mapping Results ===========================================================================');
        (0, cli_helper_1.printYellow)('\tOld Control -> New Control');
        for (const [key, value] of Object.entries(controlMappings)) {
            (0, cli_helper_1.printGreen)(`\t   ${value} -> ${key}`);
        }
        const totalMappedControls = Object.keys(controlMappings).length;
        (0, cli_helper_1.printYellowGreen)('Total Mapped Controls: ', `${totalMappedControls}\n`);
        (0, cli_helper_1.printCyan)('Control Counts ===========================');
        (0, cli_helper_1.printYellowGreen)('Total Controls Available for Delta: ', `${GenerateDelta.oldControlsLength}`);
        (0, cli_helper_1.printYellowGreen)('     Total Controls Found on XCCDF: ', `${GenerateDelta.newControlsLength}\n`);
        (0, cli_helper_1.printCyan)('Match Statistics =========================');
        (0, cli_helper_1.printYellowGreen)('                    Match Controls: ', `${GenerateDelta.match}`);
        (0, cli_helper_1.printYellowGreen)('        Possible Mismatch Controls: ', `${GenerateDelta.posMisMatch}`);
        (0, cli_helper_1.printYellowGreen)('          Duplicate Match Controls: ', `${GenerateDelta.dupMatch}`);
        (0, cli_helper_1.printYellowGreen)('                 No Match Controls: ', `${GenerateDelta.noMatch}`);
        (0, cli_helper_1.printYellowGreen)('                New XCDDF Controls: ', `${GenerateDelta.newXccdfControl}\n`);
        (0, cli_helper_1.printCyan)('Statistics Validation =============================================');
        (0, cli_helper_1.printYellowGreen)('Match + Mismatch = Total Mapped Controls: ', `${this.getMappedStatisticsValidation(totalMappedControls, 'totalMapped')}`);
        (0, cli_helper_1.printYellowGreen)('  Total Processed = Total XCCDF Controls: ', `${this.getMappedStatisticsValidation(totalMappedControls, 'totalProcessed')}\n\n`);
    ...
    createMappedDirectory(controlsDir) {
        try {
            ...
            throw new Error((0, global_1.getErrorMessage)(error), { cause: error });
        }
    }
    logThis(logMsg, logLevel) {
        switch (logLevel) {
            case 'info': {
                ...
                (0, cli_helper_1.addToProcessLogData)(logMsg);
                break;
            }
            case 'debug': {
...
async function getFlags() {
    ...
    (0, cli_helper_1.printYellow)('Provide the necessary information:');
    (0, cli_helper_1.printGreen)('  Required flag - The XCCDF XML file or URL containing the new guidance - in the form of .xml file');
    (0, cli_helper_1.printGreen)('  Required flag - Controls directory (path to the profile controls to apply the delta process)');
    (0, cli_helper_1.printGreen)('  Required flag - The output folder for the updated profile (will contain the controls that delta was applied too)');
    (0, cli_helper_1.printMagenta)('  Optional flag - InSpec Profiles JSON summary file (JSON) - auto-generated if not provided');
    (0, cli_helper_1.printMagenta)('  Optional flag - The OVAL XML file containing definitions used in the new guidance - in the form of .xml file');
    (0, cli_helper_1.printMagenta)('  Optional flag - Output markdown report file - must have an extension of .md');
    (0, cli_helper_1.printMagenta)('  Optional flag - Control ID Types: [\'rule\', \'group\', \'cis\', \'version\']');
    (0, cli_helper_1.printMagenta)('  Optional flag - Run the approximate string matching process');
    (0, cli_helper_1.printMagenta)('  Optional flag - The InSpec profile directory containing the controls being updated (controls Delta is processing)\n');
    (0, cli_helper_1.addToProcessLogData)('Process Flags ===========================================');
    ...
    if (xccdfType === 'file') {
        ...
        (0, cli_helper_1.addToProcessLogData)('xccdfXmlFile=' + xccdfXmlFile);
        ...
        (0, cli_helper_1.addToProcessLogData)('xccdfUrl=' + xccdfUrl);
    ...
    if (generateSummaryFile === 'no') {
        ...
        (0, cli_helper_1.addToProcessLogData)('inspecJsonFile=' + inspecJsonFile);
        interactiveValues.inspecJsonFile = inspecJsonFile;
    }
    else {
        (0, cli_helper_1.addToProcessLogData)('inspecJsonFile=auto-generated');
    ...
    if (useFuzzyLogic || generateSummaryFile === 'yes') {
        ...
        (0, cli_helper_1.addToProcessLogData)('runMapControls=true');
        interactiveValues.controlsDir = controlsDir;
        interactiveValues.runMapControls = useFuzzyLogic;
    }
    else {
        (0, cli_helper_1.addToProcessLogData)('runMapControls=false');
    ...
    (0, cli_helper_1.addToProcessLogData)('deltaOutputDir=' + deltaOutputDir);
    ...
    if (useOvalFile) {
        ...
        (0, cli_helper_1.addToProcessLogData)('useOvalFile=true');
        interactiveValues.ovalXmlFile = ovalXmlFile;
    }
    else {
        (0, cli_helper_1.addToProcessLogData)('useOvalFile=false');
    }
    ...
    if (generateReport) {
        ...
        (0, cli_helper_1.addToProcessLogData)('generateReport=true');
        for (const [tagName, answerValue] of Object.entries(answers)) {
            if (answerValue !== null) {
                (0, cli_helper_1.addToProcessLogData)(`${tagName}=${answerValue}`);
                interactiveValues[tagName] = answerValue;
            }
        }
    }
    else {
        (0, cli_helper_1.addToProcessLogData)('generateReport=false');
    }
        ...
        if (fs_1.default.lstatSync(xccdfXmlFile).isFile()) {
            ...
            if (path_1.default.extname(xccdfXmlFile) === '.zip') {
                ...
                if (inputFirstLine.includes('xccdf')) {
                    ...
                    (0, cli_helper_1.addToProcessLogData)(err);
                    isXccdf = false;
                }
            }
        }
...
function saveLogs(errorMsg) {
    ...
    for (const error of strArray) {
        ...
        (0, cli_helper_1.addToProcessLogData)(error.trim());
    }
    (0, cli_helper_1.saveProcessLogData)();
}
function sleep(ms) {
