...
const delta_matching_1 = require("../../utils/delta_matching");
...
class GenerateDelta extends base_command_1.BaseCommand {
    static description = 'Update an existing InSpec profile with new or updated XCCDF guidance. With -M (runMapControls), uses a 3-tier SRG/CCI requirement-first matcher for cross-vendor deltas (e.g. RHEL 9 → Amazon Linux 2023) and persists per-control match decisions into delta.json\'s links[] field.';
    static flags = {
        ...
        inspecPath: core_1.Flags.string({
    ...
    static links = [];
    logger = (0, logging_1.createWinstonLogger)({
    ...
    async run() {
        ...
        GenerateDelta.logger = (0, logging_1.createWinstonLogger)({ module: 'generate:delta', level: 'debug' });
        // Flag variables
        let inspecJsonFile;
        let inspecPath;
        let xccdfXmlFile;
        ...
        if (inspecJsonFile) {
            ...
            try {
                ...
                if (error.code === 'ENOENT') {
                    this.saveLogs(`  ERROR: File (entity) not found: ${inspecJsonFile}.\n  Run the --help command for more information on expected input files.`);
                    await this.sleep(2000).then(() => process.exit(1));
                }
                else {
                    this.saveLogs(`  ERROR: Unable to process Input execution/profile JSON ${inspecJsonFile}\n  ${error}`);
                    await this.sleep(2000).then(() => process.exit(1));
                }
            }
        }
            ...
            try {
                ...
                const inspecJsonFile = (0, global_1.safeExecFileSync)(inspecPath, ['json', path_1.default.dirname(controlsDir)], { encoding: 'utf8', maxBuffer: 50 * 1024 * 1024 });
        ...
        try {
            if (ovalXmlFile) {
                if (fs_1.default.lstatSync(ovalXmlFile).isFile()) {
                    ...
                    this.saveLogs(`  ERROR: An OVAL flag option was detected, but no file was provided\n  Ensure ${ovalXmlFile} is an OVAL file`);
                    await this.sleep(2000).then(() => process.exit(1));
                }
            }
            else {
        ...
        try {
            if (runMapControls && controlsDir) {
                ...
                for (const [key, value] of Object.entries(controls)) {
                    const controlFilename = (0, global_1.safeFilename)(`${(0, global_1.basename)(value)}.rb`);
                    const sourceShortControlFile = path_1.default.join(shortProfileDir, controlFilename);
                    const mappedShortControlFile = path_1.default.join(shortMappedDir, controlFilename);
                    const sourceControlFile = (0, global_1.resolveSafeChild)(controlsDir, controlFilename);
                    const mappedControlFile = (0, global_1.resolveSafeChild)(mappedDir, controlFilename);
                ...
                try {
                    ...
                    const inspecJsonFileNew = (0, global_1.safeExecFileSync)(inspecPath, ['json', path_1.default.dirname(mappedDir)], { encoding: 'utf8', maxBuffer: 50 * 1024 * 1024 });
            ...
            this.saveLogs('  ERROR: Could not process fuzzy search logic. Check the --help command for more '
        ...
        try {
            ...
            this.saveLogs(`  ERROR: Unable to process delta output directory: ${deltaOutputDir}\n  Check the --help command for more information on the -o flag.\n  ${error}`);
            await this.sleep(2000).then(() => process.exit(1));
        }
        // -------------------------------------------------------------------------
        ...
        if (reportFile) {
            if (fs_1.default.existsSync(reportFile) && fs_1.default.lstatSync(reportFile).isDirectory()) {
                ...
                markDownFile = (0, global_1.resolveSafeChild)(reportFile, 'delta.md');
            }
                ...
                markDownFile = (0, global_1.resolveSafeChild)(outputProfileFolderPath, 'delta.md');
            }
        }
        else {
        ...
        if (existingProfile && xccdfContent) {
            ...
            if (idTypes.includes(idType)) {
                ...
                this.saveLogs(`  ERROR: Invalid ID Type: ${idType}. Check the --help command for the available ID Type options.`);
                await this.sleep(2000).then(() => process.exit(1));
            }
            ...
            if (updatedResult) {
                for (const control of updatedResult.profile.controls) {
                    ...
                    if (index >= 0) {
                        ...
                        fs_1.default.writeFileSync((0, global_1.resolveSafeChild)(outputProfileFolderPath, 'controls', controlFilename), newControl.toRuby(processLogLevel));
                    }
                    else {
                        ...
                        const controlFilename = (0, global_1.safeFilename)(`${(0, global_1.basename)(control.id)}.rb`);
                        fs_1.default.writeFileSync((0, global_1.resolveSafeChild)(outputProfileFolderPath, 'controls', controlFilename), control.toRuby(processLogLevel));
                    }
                }
                this.logThis(`  Writing delta file for ${existingProfile.title}`, 'info');
                const deltaJsonPayload = (0, delta_matching_1.buildDeltaJsonPayload)({
                    diff: updatedResult.diff,
                    links: GenerateDelta.links,
                });
                fs_1.default.writeFileSync((0, global_1.resolveSafeChild)(outputProfileFolderPath, 'delta.json'), JSON.stringify(deltaJsonPayload, null, 2));
                if (reportFile) {
                ...
                this.saveLogs(`  ERROR: The updateProfileUsingXCCDF process failed to provide updated profiles, received: ${updatedResult}.`);
                await this.sleep(2000).then(() => process.exit(1));
            }
        }
        else {
    ...
    mapControls(oldProfile, newProfile) {
        ...
        const links = (0, delta_matching_1.applyRequirementFirstPipeline)(oldProfile, newProfile);
        GenerateDelta.links = links;
        // Cheap lookup tables for per-link logging
        const oldById = new Map(oldControls.map(c => [c.id, c]));
        const newByBasename = new Map(newControls.map(c => [(0, global_1.basename)(c.id), c]));
        for (const link of links) {
            const newId = (0, global_1.basename)(link.newId);
            ...
            if (link.matchMethod === 'none' || link.oldId === null) {
                ...
                this.logger.error(`    No Match Found for:  ${newId}${link.srg ? ` (SRG=${link.srg})` : ''}\n`);
     ...
     */
    static logMatchMethod(log, link) {
        const confidencePct = (link.confidence * 100).toFixed(0) + '%';
        switch (link.matchMethod) {
            case 'srg-deterministic': {
                log.info(`       Match method:  SRG deterministic (${link.srg}) [${link.relationship}]`);
                break;
            }
            case 'srg-cci-tiebreak': {
                log.info(`       Match method:  SRG block + CCI tiebreak (Jaccard=${confidencePct}) [${link.relationship}]`);
                break;
            }
            case 'fuse-fallback': {
                log.info(`       Match method:  Fuse title-fuzzy (no SRG overlap, confidence=${confidencePct}) [${link.relationship}]`);
                break;
            }
            default: {
    ...
    createMappedDirectory(controlsDir) {
        try {
            ...
            (0, fs_1.copyFileSync)((0, global_1.resolveSafeChild)(destFilePath, 'inspec.yml'), (0, global_1.resolveSafeChild)(path_1.default.dirname(mappedDir), 'inspec.yml'));
            return mappedDir;
        }
        catch (error) {
    ...
    async getFlags() {
        ...
        if (useFuzzyLogic || generateSummaryFile === 'yes') {
            const inspecPath = await (0, prompts_1.input)({
