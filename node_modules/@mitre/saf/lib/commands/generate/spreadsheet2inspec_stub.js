...
const cis2nist_json_1 = __importDefault(require("../../resources/cis2nist.json"));
...
class Spreadsheet2HDF extends base_command_1.BaseCommand {
    static usage = '<%= command.id %> -i, --input=<XLSX or CSV> -o, --output=FOLDER';
    static description = 'Convert CSV STIGs or CIS XLSX benchmarks into a skeleton InSpec profile';
    static examples = ['<%= config.bin %> <%= command.id %> -i spreadsheet.xlsx -o profile'];
    static flags = {
        input: core_1.Flags.string({ char: 'i', required: true, description: 'The CSV STIGs or CIS XLSX benchmarks file' }),
    ...
    matchCISControls(control, flags) {
        if (flags.format === 'cis' && control.tags?.cis_controls && typeof control.tags.cis_controls === 'string') {
            ...
            let cisControlMatches = /CONTROL:v(\d) (\d+)\.?(\d*)/.exec((control.tags.cis_controls));
    ...
    async run() {
        ...
        fs_1.default.writeFileSync((0, global_1.resolveSafeChild)(flags.output, 'inspec.yml'), yaml_1.default.stringify(profileInfo));
        // Write README.md
        ...
        fs_1.default.writeFileSync((0, global_1.resolveSafeChild)(flags.output, 'README.md'), `# ${profileInfo.name}\n${profileInfo.summary}\n---\n${yaml_1.default.stringify(readableMetadata)}`);
        try {
            const workBook = await xlsx_populate_1.default.fromFileAsync(flags.input);
                ...
                if (usedRange) {
                    ...
                    const mappedRecords = extractedData.slice(1).map((row) => {
                        ...
                        for (const [index, cellValue] of row.entries()) {
                            if (lodash_1.default.isString(cellValue)) {
                                mappedRecord[headers[index]] = cellValue;
                            }
                            else if (lodash_1.default.isNumber(cellValue)) {
            ...
            const records = (0, sync_1.parse)(inputDataLines.join('\n'), {
                columns: true,
        ...
        for (const control of inspecControls) {
            fs_1.default.writeFileSync((0, global_1.resolveSafeChild)(flags.output, 'controls', (0, global_1.safeFilename)(`${(0, global_1.basename)(control.id)}.rb`)), (0, xccdf2inspec_1.inspecControlToRubyCode)(control, flags.lineLength, flags.encodingHeader));
        }
    }
}
