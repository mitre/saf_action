...
class GenerateThreshold extends base_command_1.BaseCommand {
    static usage = '<%= command.id %> -i <hdf-json> [-o <threshold-yaml>] [-h] [-e] [-c]';
    ...
    static examples = ['<%= config.bin %> <%= command.id %> -i rhel7-results.json -e -c -o output.yaml'];
    static flags = {
        input: core_1.Flags.string({ char: 'i', required: true, description: 'Input HDF JSON File' }),
        output: core_1.Flags.string({ char: 'o', required: false, description: 'Output Threshold YAML File' }),
        ...
        generateControlIds: core_1.Flags.boolean({ char: 'c', required: false, description: 'Validate control IDs have the correct severity and status' }),
    };
    async run() {
        ...
        if (flags.output) {
            ...
            console.log(yaml_1.default.stringify(thresholds));
        }
    }
}
