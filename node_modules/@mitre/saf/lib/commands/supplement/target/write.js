...
class WriteTarget extends baseCommand_1.BaseCommand {
    static usage = '<%= command.id %> -i <input-hdf-json> (-f <input-target-json> | -d <target-json>) [-o <output-hdf-json>]';
    ...
    async run() {
        ...
        if (flags.targetFile) {
            try {
                ...
                throw new Error(`Couldn't parse target data: ${error instanceof Error ? error.message : JSON.stringify(error)}`);
            }
        }
