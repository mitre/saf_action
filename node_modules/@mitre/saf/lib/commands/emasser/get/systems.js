...
class EmasserGetSystems extends core_1.Command {
    async run() {
        ...
        getSystems.getSystems(flags.includePackage, flags.registrationType, flags.ditprId, flags.coamsId, flags.policy, flags.includeDitprMetrics, flags.includeDecommissioned, flags.reportsForScorecard).then((response) => {
...
EmasserGetSystems.usage = '<%= command.id %> [ARGUMENTS]';
EmasserGetSystems.description = 'Get available systems filter on provided options';
EmasserGetSystems.examples = ['<%= config.bin %> <%= command.id %> [options]'];
EmasserGetSystems.flags = {
    help: core_1.Flags.help({ char: 'h', description: 'Show emasser CLI help for the GET Systems endpoint' }),
    ...(0, utilities_1.getFlagsForEndpoint)(process.argv), // skipcq: JS-0349
};
exports.default = EmasserGetSystems;
