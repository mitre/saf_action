...
class EmasserGetCac extends core_1.Command {
    async run() {
        const { flags } = await this.parse(EmasserGetCac);
        const apiCxn = new apiConnection_1.ApiConnection();
        const getCac = new emass_client_1.CACApi(apiCxn.configuration, apiCxn.basePath, apiCxn.axiosInstances);
        // Order is important here
        getCac.getSystemCac(flags.systemId, flags.controlAcronyms).then((response) => {
...
exports.default = EmasserGetCac;
EmasserGetCac.usage = '<%= command.id %> [ARGUMENTS]';
EmasserGetCac.description = 'View one or many Control Approval Chain (CAC) in a system specified system ID';
EmasserGetCac.examples = ['<%= config.bin %> <%= command.id %> --systemId <value>'];
EmasserGetCac.flags = {
    help: core_1.Flags.help({ char: 'h', description: 'Show emasser CLI help for the GET CAC endpoint' }),
