...
class EmasserGetPac extends core_1.Command {
    async run() {
        const { flags } = await this.parse(EmasserGetPac);
        const apiCxn = new apiConnection_1.ApiConnection();
        const getPac = new emass_client_1.PACApi(apiCxn.configuration, apiCxn.basePath, apiCxn.axiosInstances);
        // Order is important here
        getPac.getSystemPac(flags.systemId).then((response) => {
...
EmasserGetPac.usage = '<%= command.id %> [options]';
EmasserGetPac.description = 'View one or many Package Approval Chain (PAC) in a system specified system ID';
EmasserGetPac.examples = ['<%= config.bin %> <%= command.id %> --systemId <value>'];
EmasserGetPac.flags = {
    help: core_1.Flags.help({ char: 'h', description: 'Show emasser CLI help for the GET PAC endpoint' }),
    ...(0, utilities_1.getFlagsForEndpoint)(process.argv), // skipcq: JS-0349
};
exports.default = EmasserGetPac;
