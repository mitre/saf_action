...
class EmasserGetCmmc extends core_1.Command {
    async run() {
        ...
        getCmmc.getCmmcAssessments(flags.sinceDate).then((response) => {
...
EmasserGetCmmc.usage = '<%= command.id %> [ARGUMENTS]';
EmasserGetCmmc.description = 'View Cybersecurity Maturity Model Certification (CMMC) Assessments';
EmasserGetCmmc.examples = ['<%= config.bin %> <%= command.id %> --sinceDate <value>'];
EmasserGetCmmc.flags = {
    help: core_1.Flags.help({ char: 'h', description: 'Show emasser CLI help for the GET CMMC endpoint' }),
