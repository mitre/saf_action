...
class EmasserContainerScans extends core_1.Command {
    ...
    static examples = ['<%= config.bin %> <%= command.id %> [-s,--systemId] [-c,--containerId] <container-id> <container-id> ...'];
    static flags = {
        help: core_1.Flags.help({ char: 'h', description: 'Show help for the SAF CLI eMASSer DELETE Container Scans command' }),
    ...
    async run() {
        ...
        const containerScan = new emass_client_1.ContainerScanResultsApi(apiCxn.configuration, apiCxn.basePath, apiCxn.axiosInstances);
        const requestBodyArray = [];
        flags.containerId.forEach((containerId) => {
            requestBodyArray.push({ containerId: containerId.replace(',', '') });
        });
        // Call the API
        containerScan.deleteContainerSans(flags.systemId, requestBodyArray).then((response) => {
