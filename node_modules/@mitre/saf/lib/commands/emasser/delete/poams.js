...
class EmasserDeletePoams extends core_1.Command {
    async run() {
        ...
        flags.poamsId.forEach((poamId) => {
            requestBodyArray.push({ poamId: poamId }); // skipcq: JS-0240
        });
        delPoam.deletePoam(flags.systemId, requestBodyArray).then((response) => {
...
EmasserDeletePoams.usage = '<%= command.id %> [ARGUMENTS]';
EmasserDeletePoams.description = 'Remove one or many POA&M items in a system identified by system and poam Id';
EmasserDeletePoams.examples = ['<%= config.bin %> <%= command.id %> [-s,--systemId] [-P,--poamsId]'];
EmasserDeletePoams.flags = {
