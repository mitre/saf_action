...
function addRequiredFieldsToRequestBody(dataObj) {
    ...
    try {
        ...
        console.log((0, json_colorizer_1.default)(JSON.stringify((0, utilities_1.getJsonExamples)('cloud_resources-required'), null, 2)));
        throw error;
    }
...
class EmasserPostCloudResources extends core_1.Command {
    async run() {
        ...
        const addCloudResource = new emass_client_1.CloudResourcesApi(apiCxn.configuration, apiCxn.basePath, apiCxn.axiosInstances);
        const requestBodyArray = [];
        // Check if a Cloud Resource json file was provided
        if (fs_1.default.existsSync(flags.cloudResourceFile)) {
            let data;
            try {
                data = JSON.parse(await (0, promises_1.readFile)(flags.cloudResourceFile, 'utf8'));
            }
            catch (error) {
...
EmasserPostCloudResources.usage = '<%= command.id %> [ARGUMENTS]';
EmasserPostCloudResources.description = 'Add a cloud resource and their scan results in the assets module for a system';
EmasserPostCloudResources.examples = ['<%= config.bin %> <%= command.id %> [-s,--systemId] [-f,--cloudResourceFile]',
    ...
    (0, json_colorizer_1.default)(JSON.stringify((0, utilities_1.getJsonExamples)('cloud_resources-required'), null, 2)),
    'Optional JSON parameters/fields are:',
    (0, json_colorizer_1.default)(JSON.stringify((0, utilities_1.getJsonExamples)('cloud_resources-optional'), null, 2))];
EmasserPostCloudResources.flags = {
    help: core_1.Flags.help({ char: 'h', description: 'Post (add) cloud resources and their scan results in the assets module for a system' }),
