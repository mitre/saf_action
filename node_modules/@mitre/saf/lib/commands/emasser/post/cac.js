...
class EmasserPostCac extends core_1.Command {
    async run() {
        ...
        addCac.addSystemCac(flags.systemId, requestBodyArray).then((response) => {
...
EmasserPostCac.usage = '<%= command.id %> [ARGUMENTS]';
EmasserPostCac.description = 'Add a Control Approval Chain (CAC) items in a system';
EmasserPostCac.examples = ['<%= config.bin %> <%= command.id %> [-s,--systemId] [-a,--controlAcronym] [options]'];
EmasserPostCac.flags = {
    help: core_1.Flags.help({ char: 'h', description: 'Post (add) control to second stage of CAC' }),
