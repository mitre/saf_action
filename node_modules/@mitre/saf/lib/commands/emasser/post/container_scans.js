    ...
    try {
        ...
        dataObj.benchmarks.forEach((entryObject) => {
            ...
            entryObject.results.forEach((resultObj) => {
                ...
                j++;
                const resultsObj = { ruleId: resultObj.ruleId, lastSeen: resultObj.lastSeen, status: resultObj.status };
...
function addOptionalFields(bodyObject, dataObj) {
    ...
    // eslint-disable-next-line unicorn/no-array-for-each
    dataObj.benchmarks.forEach((entryObject) => {
        const benchmarksObj = { benchmark: entryObject.benchmark, results: [] };
        // These are required
        ...
        entryObject.results.forEach((resultObj) => {
            // These are required
            const resultsObj = { ruleId: resultObj.ruleId, status: resultObj.status, lastSeen: resultObj.lastSeen };
 ...
 */
function isValidContainerResource(obj) {
...
class EmasserContainerScans extends core_1.Command {
    ...
    async run() {
        ...
        */
        function processContainerResource(dataObject) {
            try {
                const bodyObj = addRequiredFieldsToRequestBody(dataObject);
        ...
        try {
            ...
            (0, utilities_1.displayError)(error, 'Container Scans');
        }
    }
    catch(err) {
