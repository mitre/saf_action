...
class EmasserPutMilestones extends core_1.Command {
    async run() {
        const { flags } = await this.parse(EmasserPutMilestones);
        const apiCxn = new apiConnection_1.ApiConnection();
        const putMilestones = new emass_client_1.MilestonesApi(apiCxn.configuration, apiCxn.basePath, apiCxn.axiosInstances);
        ...
        putMilestones.updateMilestoneBySystemIdAndPoamId(flags.systemId, flags.poamId, requestBodyArray).then((response) => {
...
EmasserPutMilestones.usage = '<%= command.id %> [ARGUMENTS]';
EmasserPutMilestones.description = 'Update milestone(s) for specified system, poam, and milestone Id';
EmasserPutMilestones.examples = ['<%= config.bin %> <%= command.id %> [-s,--systemId] [-p,--poamId] [-m,--milestoneId] [-d,--description] [-c,--scheduledCompletionDate]'];
EmasserPutMilestones.flags = {
    help: core_1.Flags.help({ char: 'h', description: 'Show emasser CLI help for the PUT Milestones endpoint' }),
    ...(0, utilities_1.getFlagsForEndpoint)(process.argv), // skipcq: JS-0349
};
exports.default = EmasserPutMilestones;
