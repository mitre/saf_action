...
class Heimdall extends core_1.Command {
    ...
    static usage = '<%= command.id %> [-h] [-p <port>] [-f <file>] [-n]';
    static description = 'Run an instance of Heimdall Lite to visualize your data';
    static examples = ['<%= config.bin %> <%= command.id %> -p 8080'];
    static flags = {
        ...
        noOpenBrowser: core_1.Flags.boolean({ char: 'n', required: false, default: false, description: 'Do not open the default browser automatically' }),
    };
    async run() {
        ...
        const openDynamicImport = await (0, tsimportlib_1.dynamicImport)('open', module);
        const open = openDynamicImport.default;
        const { flags } = await this.parse(Heimdall);
        let parsedJSONs = [];
        // Is the defined port valid?
        if (Number.isNaN(flags.port) || flags.port < 1 || flags.port >= 65_536) {
        ...
        const installedPath = (0, global_1.getInstalledPath)('@mitre/saf');
        (0, express_1.default)()
