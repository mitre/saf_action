...
class Heimdall extends command_1.Command {
    async run() {
        ...
        if (flags.files && flags.files.length > 0) {
            ...
            parsedJSONs = flags.files.map(file => {
                return { filename: path_1.default.parse(file).base, data: fs_1.default.readFileSync(file, 'utf-8') };
            });
        }
...
Heimdall.flags = {
    ...
    port: command_1.flags.integer({ char: 'p', required: false, default: 3000, description: 'Port To Expose Heimdall On (Default 3000)' }),
    files: command_1.flags.string({ char: 'f', required: false, multiple: true, description: 'File(s) to display in Heimdall' }),
    noOpenBrowser: command_1.flags.boolean({ char: 'n', required: false, default: false, description: "Don't open the default browser automatically" }),
};
