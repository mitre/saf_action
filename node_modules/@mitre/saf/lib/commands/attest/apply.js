...
class ApplyAttestation extends base_command_1.BaseCommand {
    static usage = '<%= command.id %> -i <input-hdf-json>... <attestation>... -o <output-hdf-path>';
    ...
    async run() {
        ...
        for (const inputFile of flags.input) {
            ...
            try {
                ...
                if (Array.isArray(inputData) && inputData.length > 0 && lodash_1.default.get(inputData, '[0].control_id')) {
                    ...
                    executions[(0, global_1.basename)(inputFile)] = inputData;
                }
                else {
                    // Unknown file
        ...
        for (const [originalFilename, execution] of Object.entries(executions)) {
            ...
            if (Object.entries(executions).length <= 1) {
                ...
                fs_1.default.writeFileSync((0, global_1.resolveSafeChild)(flags.output, (0, global_1.safeFilename)(originalFilename)), JSON.stringify(applied, null, 2));
            }
        }
    }
}
