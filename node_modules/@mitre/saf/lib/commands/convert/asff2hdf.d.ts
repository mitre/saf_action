import { BaseCommand } from '../../utils/oclif/base_command';
export default class ASFF2HDF extends BaseCommand<typeof ASFF2HDF> {
    static readonly usage: string;
    static readonly description = "Translate a AWS Security Finding Format JSON into a Heimdall Data Format JSON file(s)";
    static readonly examples: {
        description: string;
        command: string;
    }[];
    static readonly flags: {
        input: import("@oclif/core/interfaces").OptionFlag<string | undefined, import("@oclif/core/interfaces").CustomOptions>;
        aws: import("@oclif/core/interfaces").BooleanFlag<boolean>;
        region: import("@oclif/core/interfaces").OptionFlag<string | undefined, import("@oclif/core/interfaces").CustomOptions>;
        insecure: import("@oclif/core/interfaces").BooleanFlag<boolean>;
        securityHub: import("@oclif/core/interfaces").OptionFlag<string[] | undefined, import("@oclif/core/interfaces").CustomOptions>;
        output: import("@oclif/core/interfaces").OptionFlag<string, import("@oclif/core/interfaces").CustomOptions>;
        certificate: import("@oclif/core/interfaces").OptionFlag<string | undefined, import("@oclif/core/interfaces").CustomOptions>;
        target: import("@oclif/core/interfaces").OptionFlag<string[] | undefined, import("@oclif/core/interfaces").CustomOptions>;
    };
    run(): Promise<void>;
}
//# sourceMappingURL=asff2hdf.d.ts.map