...
function processInputs(scoreDoc, profiles, output, withRaw) {
    ...
    for (const hdfReport of converter.toHdf()) {
        const outputBase = path_1.default.dirname(output);
        const outputPrefix = (0, global_1.safeFilename)(output.replaceAll(/\.json/gi, ''));
        ...
        fs_1.default.writeFileSync((0, global_1.resolveSafeChild)(outputBase, (0, global_1.safeFilename)(`${outputPrefix}-${(0, global_1.basename)(reportId)}.json`)), JSON.stringify(hdfReport));
    }
}
class MsftSecure2HDF extends base_command_1.BaseCommand {
    static usage = [
