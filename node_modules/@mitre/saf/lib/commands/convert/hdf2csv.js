...
const stringify = __importStar(require("csv-stringify"));
...
class HDF2CSV extends base_command_1.BaseCommand {
    ...
    async run() {
        ...
        (0, cli_helper_1.addToProcessLogData)('================== HDF2CSV CLI Process ===================');
        ...
        if (flags.interactive) {
            ...
            for (const [key, value] of Object.entries(flags)) {
                (0, cli_helper_1.addToProcessLogData)(`${key}=${String(value)}`);
            }
        }
        else {
            return;
        ...
        if (validFileFlags(inputFile, outputFile)) {
            ...
            let rows = convertRows(contextualizedEvaluation, (0, global_1.basename)(inputFile), includeFields.split(','));
                ...
                for (const [key, value] of Object.entries(row)) {
                    if (value !== undefined) {
                        if (value.length > 32_767 && truncateFields) {
                            ...
                            cleanedRow[key] = lodash_1.default.truncate(value, { length: 32_757, omission: 'TRUNCATED' });
                        }
                        else {
...
async function saveCSV(filename, data) {
    ...
    try {
        ...
        try {
            await fs_1.promises.writeFile(filename, csvData, 'utf8');
        }
        catch (error) {
...
async function getFlags() {
    ...
    (0, cli_helper_1.printGreen)('  Required flag - HDF file to convert to a CSV formatted file');
    (0, cli_helper_1.printGreen)('  Required flag - Translation output directory (where the CSV file is written to)');
    (0, cli_helper_1.printGreen)('  Required flag - CSV output file name (default name is hdf2csv.csv)');
    (0, cli_helper_1.printGreen)('  Required flag - Field(s) (at least one) to include in output CSV (comma delineated)');
    (0, cli_helper_1.printMagenta)('  Optional flag - Truncate fields that exceed Excel cell limit (32,767 characters)\n');
    const choices = [];
    ...
    for (const [tagName, answerValue] of Object.entries(answers)) {
        if (answerValue !== null) {
            (0, cli_helper_1.addToProcessLogData)(`${tagName}=${lodash_1.default.isArray(answerValue) ? answerValue.join(',') : String(answerValue)}`);
        }
    }
    return answers;
}
