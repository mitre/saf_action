...
class HDF2CKL extends command_1.Command {
    async run() {
        const { flags } = this.parse(HDF2CKL);
        const contextualizedEvaluation = (0, inspecjs_1.contextualizeEvaluation)(JSON.parse(fs_1.default.readFileSync(flags.input, { encoding: 'utf-8' })));
        ...
        if (flags.metadata) {
            const cklMetadataInput = JSON.parse(fs_1.default.readFileSync(flags.metadata, 'utf-8'));
...
HDF2CKL.flags = {
    ...
    metadata: command_1.flags.string({ char: 'm', required: false, description: 'Metadata JSON file, generate one with "saf generate:ckl_metadata"' }),
    output: command_1.flags.string({ char: 'o', required: true, description: 'Output CKL file' }),
    hostname: command_1.flags.string({ char: 'H', required: false, description: 'Hostname for CKL metadata' }),
    fqdn: command_1.flags.string({ char: 'F', required: false, description: 'FQDN for CKL metadata' }),
    mac: command_1.flags.string({ char: 'M', required: false, description: 'MAC address for CKL metadata' }),
    ip: command_1.flags.string({ char: 'I', required: false, description: 'IP address for CKL metadata' }),
};
