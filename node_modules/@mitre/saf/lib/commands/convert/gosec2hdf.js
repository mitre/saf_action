...
class Gosec2HDF extends base_command_1.BaseCommand {
    ...
    static flags = {
        ...
        includeRaw: core_1.Flags.boolean({ char: 'w', required: false, description: 'Include raw input file in HDF JSON file' }),
    };
    async run() {
        const { flags } = await this.parse(Gosec2HDF);
        // Check for correct input type
        const data = fs_1.default.readFileSync(flags.input, 'utf8');
        (0, global_1.checkInput)({ data, filename: flags.input }, hdf_converters_1.INPUT_TYPES.GOSEC, 'gosec results JSON');
        const converter = new hdf_converters_1.GosecMapper(data, flags.includeRaw);
...
//# sourceMappingURL=gosec2hdf.js.map