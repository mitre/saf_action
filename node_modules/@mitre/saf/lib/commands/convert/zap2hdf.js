...
class Zap2HDF extends baseCommand_1.BaseCommand {
    static usage = '<%= command.id %> -i <zap-json> -n <target-site-name> -o <hdf-scan-results-json> [-h] [-w]';
    static description = 'Translate a OWASP ZAP results JSON to a Heimdall Data Format JSON file';
    static examples = ['<%= config.bin %> <%= command.id %> -i zap_results.json -n mitre.org -o scan_results.json'];
    static flags = {
    ...
    async run() {
        ...
        (0, global_1.checkInput)({ data, filename: flags.input }, 'zap', 'OWASP ZAP results JSON');
        const converter = new hdf_converters_1.ZapMapper(fs_1.default.readFileSync(flags.input, 'utf8'), flags.name, flags.includeRaw);
...
//# sourceMappingURL=zap2hdf.js.map