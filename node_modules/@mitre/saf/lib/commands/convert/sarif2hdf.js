...
class Sarif2HDF extends core_1.Command {
    async run() {
        ...
        (0, global_1.checkInput)({ data: data, filename: flags.input }, 'sarif', 'SARIF JSON');
        const converter = new hdf_converters_1.SarifMapper(data);
        fs_1.default.writeFileSync((0, global_1.checkSuffix)(flags.output), JSON.stringify(converter.toHdf()));
    }
}
Sarif2HDF.usage = 'convert sarif2hdf -i <sarif-json> -o <hdf-scan-results-json> [-h]';
...
Sarif2HDF.flags = {
    ...
    input: core_1.Flags.string({ char: 'i', required: true, description: 'Input SARIF JSON File' }),
