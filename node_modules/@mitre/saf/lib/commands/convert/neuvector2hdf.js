...
class NeuVector2HDF extends baseCommand_1.BaseCommand {
    static usage = '<%= command.id %> -i <neuvector-json> -o <hdf-scan-results-json>';
    static description = 'Translate a NeuVector results JSON to a Heimdall Data Format JSON file';
    static examples = ['<%= config.bin %> <%= command.id %> -i neuvector.json -o output-hdf-name.json'];
    static flags = {
    ...
    async run() {
        ...
        const converter = new hdf_converters_1.NeuVectorMapper(input, flags.includeRaw);
