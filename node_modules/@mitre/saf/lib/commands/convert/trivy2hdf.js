...
class Trivy2HDF extends command_1.Command {
    async run() {
        ...
        let input = fs_1.default.readFileSync(flags.input, { encoding: 'utf-8' }).trim();
        if (Array.isArray(JSON.parse(input))) {
            input = `{"Findings": ${fs_1.default.readFileSync(flags.input, { encoding: 'utf-8' }).trim()}}`;
        }
        const meta = { name: 'Trivy', title: 'Trivy Findings' };
        const converter = new hdf_converters_1.ASFFMapper(input, undefined, meta);
