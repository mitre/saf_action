...
class Burpsuite2HDF extends baseCommand_1.BaseCommand {
    static usage = '<%= command.id %> -i <burpsuite-xml> -o <hdf-scan-results-json> [-h] [-w]';
    static description = 'Translate a BurpSuite Pro XML file into a Heimdall Data Format JSON file';
    static examples = ['<%= config.bin %> <%= command.id %> -i burpsuite_results.xml -o output-hdf-name.json'];
    static flags = {
    ...
    async run() {
        ...
        (0, global_1.checkInput)({ data, filename: flags.input }, 'burp', 'BurpSuite Pro XML');
        const converter = new hdf_converters_1.BurpSuiteMapper(data, flags.includeRaw);
...
//# sourceMappingURL=burpsuite2hdf.js.map