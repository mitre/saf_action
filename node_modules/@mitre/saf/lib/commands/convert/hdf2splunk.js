...
class HDF2Splunk extends base_command_1.BaseCommand {
    static usage = '<%= command.id %> -i <hdf-scan-results-json> -H <host> -I <index> [-h] [-P <port>] [-s http|https] [-u <username> | -t <token>] [-p <password>] [-L info|warn|debug|verbose]';
    ...
    async run() {
        ...
        const logger = (0, logging_1.createWinstonLogger)({ module: 'hdf2splunk', level: flags.logLevel });
        ...
        logger.info(`Input File "${(0, global_1.basename)(flags.input)}": ${(0, logging_1.getHDFSummary)(inputFile)}`);
        await new hdf_converters_1.FromHDFToSplunkMapper(inputFile, logger).toSplunk({
            ...
            index: flags.index,
        }, (0, global_1.basename)(flags.input));
    }
}
exports.default = HDF2Splunk;
//# sourceMappingURL=hdf2splunk.js.map