...
const prompt_sync_1 = (0, tslib_1.__importDefault)(require("prompt-sync"));
...
class CKL2POAM extends command_1.Command {
    async run() {
                ...
                parser.parseString(data, function (parseFileError, result) {
                    if (parseFileError) {
                        ...
                        xlsx_populate_1.default.fromDataAsync((0, global_1.dataURLtoU8Array)(files_json_1.default.POAMTemplate.data)).then((workBook) => {
                            ...
                            return workBook.toFileAsync(path_1.default.join(flags.output, `${fileName}-${(0, moment_1.default)(new Date()).format('YYYY-MM-DD-HHmm')}.xlsm`));
                        });
                    }
                });
            });
...
CKL2POAM.flags = {
    ...
    input: command_1.flags.string({ char: 'i', required: true, multiple: true, description: 'Path to the DISA Checklist File(s)' }),
    officeOrg: command_1.flags.string({ char: 'O', required: false, default: '', description: 'Default value for Office/org (prompts for each file if not set)' }),
    deviceName: command_1.flags.string({ char: 'd', required: false, default: '', description: 'Name of target device (prompts for each file if not set)' }),
    rowsToSkip: command_1.flags.integer({ char: 's', required: false, default: 4, description: 'Rows to leave between POA&M Items for milestones' }),
    output: command_1.flags.string({ char: 'o', required: true, description: 'Path to output PO&M File(s)' }),
};
