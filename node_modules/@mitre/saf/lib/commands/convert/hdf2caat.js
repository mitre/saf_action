...
class HDF2CAAT extends core_1.Command {
    ...
    static flags = {
        ...
        input: core_1.Flags.string({ char: 'i', required: true, multiple: true, description: 'Input HDF JSON file' }),
        output: core_1.Flags.string({ char: 'o', required: true, description: 'Output CAAT XLSX file' }),
    };
    async run() {
        const { flags } = await this.parse(HDF2CAAT);
        const inputData = flags.input.map(filename => ({ data: fs_1.default.readFileSync(filename, 'utf8'), filename: path_1.default.basename(filename) }));
