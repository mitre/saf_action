...
class JfrogXray2HDF extends baseCommand_1.BaseCommand {
    static usage = '<%= command.id %> -i <jfrog-xray-json> -o <hdf-scan-results-json> [-h] [-w]';
    static description = 'Translate a JFrog Xray results JSON file into a Heimdall Data Format JSON file';
    static examples = ['<%= config.bin %> <%= command.id %> -i xray_results.json -o output-hdf-name.json'];
    static flags = {
    ...
    async run() {
        ...
        (0, global_1.checkInput)({ data, filename: flags.input }, 'jfrog', 'JFrog Xray results JSON');
        const converter = new hdf_converters_1.JfrogXrayMapper(data, flags.includeRaw);
...
//# sourceMappingURL=jfrog_xray2hdf.js.map