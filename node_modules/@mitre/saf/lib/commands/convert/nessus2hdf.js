...
class Nessus2HDF extends core_1.Command {
    async run() {
        const { flags } = await this.parse(Nessus2HDF);
        // Check for correct input type
        const data = fs_1.default.readFileSync(flags.input, 'utf8');
        (0, global_1.checkInput)({ data: data, filename: flags.input }, 'nessus', 'Nessus XML results file');
        const converter = new hdf_converters_1.NessusResults(data);
...
Nessus2HDF.usage = 'convert nessus2hdf -i <nessus-xml> -o <hdf-scan-results-json> [-h]';
...
Nessus2HDF.flags = {
    ...
    input: core_1.Flags.string({ char: 'i', required: true, description: 'Input Nessus XML File' }),
