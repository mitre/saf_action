...
async function processPrompt() {
    ...
    const interactiveValues = { EMASSER_KEY_FILE_PATH: '', EMASSER_CERT_FILE_PATH: '', EMASSER_CA_FILE_PATH: '' };
    ...
    for (const tagName in requiredContent) {
        if (Object.prototype.hasOwnProperty.call(requiredContent, tagName)) {
            const answerValue = lodash_1.default.get(requiredContent, tagName);
            if (answerValue !== null) {
    ...
    const certTypes = {
        certType: await (0, prompts_1.select)({
    ...
    for (const tagName in requiredCerts) {
        if (Object.prototype.hasOwnProperty.call(requiredCerts, tagName)) {
            const answerValue = lodash_1.default.get(requiredCerts, tagName);
            if (answerValue !== null) {
                interactiveValues[tagName] = answerValue;
            }
        }
    ...
    const addUserUid = await (0, prompts_1.confirm)({ message: 'Add a unique user identifier (user-id)?' });
    if (addUserUid) {
        const EMASSER_USER_UID = await (0, prompts_1.input)({
    ...
    const optionalContent = {
        ...
        EMASSER_DOWNLOAD_DIR: await (0, prompts_1.input)({
    ...
    for (const tagName in optionalContent) {
        if (Object.prototype.hasOwnProperty.call(optionalContent, tagName)) {
            const answerValue = lodash_1.default.get(optionalContent, tagName);
            if (answerValue !== null) {
                interactiveValues[tagName] = answerValue;
            }
        }
    ...
    for (const [key, value] of Object.entries(envData)) {
        ...
        if (key.startsWith('#')) {
            console.log(`\t${colors_1.default.green(key)}`);
        }
        else if (key.trim() !== '') {
            console.log(`\t${colors_1.default.blue(key)}=${colors_1.default.dim(value)}`);
        }
    }
 ...
 *
 */
function updateKeyValuePairs(filePath, updates) {
    try {
        ...
        fs_extra_1.default.writeFileSync(filePath, updatedContent, 'utf8');
