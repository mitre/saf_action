...
exports.printHelpMsg = printHelpMsg;
exports.printRedMsg = printRedMsg;
exports.displayError = displayError;
const fs_1 = __importDefault(require("fs"));
const core_1 = require("@oclif/core");
const json_colorizer_1 = require("json-colorizer");
const output_error_1 = require("./output_error");
        ...
        case 'get': {
            switch (args.endpoint) { // skipcq: JS-0047
                ...
                case 'roles': {
                    if (args.argument === 'byCategory') {
                        flagObj = {
                            ...
                            role: core_1.Flags.string({ char: 'r', description: 'Accepts single value from options available at base system-roles endpoint e.g., SCA', required: true }),
                            policy: core_1.Flags.string({ char: 'p', description: 'Filter on policy', options: ['diacap', 'rmf', 'reporting'], required: false }),
                        };
                    }
                    break;
                ...
                case 'controls': {
                    flagObj = {
                        systemId: core_1.Flags.integer({ char: 's', description: 'The system identification number', required: true }),
                        acronyms: core_1.Flags.string({ char: 'A', description: 'The system acronym(s) e.g "AC-1, AC-2" - if not provided all controls for systemId are returned', required: false }),
                    };
                    break;
                }
                case 'test_results': {
                    flagObj = {
                        ...
                        assessmentProcedures: core_1.Flags.string({ char: 'p', description: 'The system Security Control Assessment Procedure e.g "AC-1.1,AC-1.2', required: false }),
                ...
                case 'poams': {
                    if (args.argument === 'forSystem') {
                        flagObj = {
                            systemId: core_1.Flags.integer({ char: 's', description: 'The system identification number', required: true }),
                            scheduledCompletionDateStart: core_1.Flags.string({ char: 'd', description: 'The scheduled completion start date', required: false }),
                            scheduledCompletionDateEnd: core_1.Flags.string({ char: 'e', description: 'The scheduled completion end date', required: false }),
                            controlAcronyms: core_1.Flags.string({ char: 'a', description: 'The system acronym(s) e.g "AC-1, AC-2"', required: false }),
                            assessmentProcedures: core_1.Flags.string({ char: 'p', description: 'The system Security Control Assessment Procedure e.g "AC-1.1,AC-1.2', required: false }),
                            ccis: core_1.Flags.string({ char: 'c', description: 'The system CCIS string numerical value', required: false }),
                ...
                case 'artifacts': {
                    if (args.argument === 'forSystem') {
                        ...
                        flagObj = {
                            ...
                            printToStdOut: core_1.Flags.boolean({ char: 'P', description: 'Boolean - Print to standard output', allowNo: true, required: false }),
                        };
                    }
                    break;
                }
                ...
                case 'workflow_instances': {
                    if (args.argument === 'all') {
                        flagObj = {
                            ...
                            pageIndex: core_1.Flags.integer({ char: 'i', description: 'The page number to query (default first page 0)', required: false }),
        ...
        case 'post': {
            switch (args.endpoint) { // skipcq: JS-0047
                case 'test_results': {
                    flagObj = {
                        systemId: core_1.Flags.integer({ char: 's', description: 'The system identification number', required: true }),
                        assessmentProcedure: core_1.Flags.string({ char: 'a', description: 'The Security Control Assessment Procedure being assessed', required: true }),
                        ...
                        complianceStatus: core_1.Flags.string({ char: 'S', description: 'The compliance status of the test result',
                ...
                case 'artifacts': {
                    flagObj = {
                        systemId: core_1.Flags.integer({ char: 's', description: 'The system identification number', required: true }),
                        fileName: core_1.Flags.string({ char: 'f', description: 'Artifact file(s) to post to the given system, can have multiple (space separated)', required: true, multiple: true }),
                        isTemplate: core_1.Flags.boolean({ char: 'T', description: 'Boolean - Indicates whether an artifact is a template.', allowNo: true, required: false, default: false }),
                        type: core_1.Flags.string({ char: 't', description: 'Various artifact file type are accepted (defined by the eMASS administrator)', required: false, default: 'Other' }),
                        category: core_1.Flags.string({ char: 'c', description: 'Various artifact category are accepted (defined by the eMASS administrator)', required: false, default: 'Evidence' }),
                    };
                    break;
                }
                case 'cac': {
                ...
                case 'container_scans': {
                    flagObj = {
                        systemId: core_1.Flags.integer({ char: 's', description: 'The system identification number', required: true }),
                        dataFile: core_1.Flags.string({ char: 'f', description: 'A well formed JSON file containing the data to add. It can ba a single object or an array of objects.', required: true }),
                    };
                    break;
                }
            }
        ...
        case 'put': {
            switch (args.endpoint) { // skipcq: JS-0047
                ...
                case 'software_baseline': {
                    flagObj = {
                        systemId: core_1.Flags.integer({ char: 's', description: 'The system identification number', required: true }),
                        dataFile: core_1.Flags.string({ char: 'f', description: 'A well formed JSON file containing the data to be updated. It can ba a single object or an array of objects.', required: true }),
                    };
                    break;
                }
            }
        ...
        case 'delete': {
            switch (args.endpoint) { // skipcq: JS-0047
                ...
                case 'hardware_baseline': {
                    flagObj = {
                        systemId: core_1.Flags.integer({ char: 's', description: 'The system identification number', required: true }),
                        assetsHardwareId: core_1.Flags.string({ char: 'a', description: 'Unique GUID identifying a specific hardware asset, can have multiple (space separated)', required: true, multiple: true }),
                    };
                    break;
                }
                case 'software_baseline': {
                    flagObj = {
                        systemId: core_1.Flags.integer({ char: 's', description: 'The system identification number', required: true }),
                        assetsSoftwareId: core_1.Flags.string({ char: 'a', description: 'Unique GUID identifying a specific software asset, can have multiple (space separated)', required: true, multiple: true }),
                    };
                    break;
                }
                case 'cloud_resources': {
                    flagObj = {
                        systemId: core_1.Flags.integer({ char: 's', description: 'The system identification number', required: true }),
                        resourceId: core_1.Flags.string({ char: 'r', description: 'Unique identifier/resource namespace for policy compliance result, can have multiple (space separated)', required: true, multiple: true }),
                    };
                    break;
                }
                case 'container_scans': {
                    flagObj = {
                        systemId: core_1.Flags.integer({ char: 's', description: 'The system identification number', required: true }),
                        containerId: core_1.Flags.string({ char: 'c', description: 'Unique identifier of the container, can have multiple (space separated)', required: true, multiple: true }),
                    };
                    break;
                }
            }
...
function saveFile(dir, filename, data) {
    ...
    // Save to file
    const outputFile = (0, global_1.resolveSafeChild)(dir, (0, global_1.safeFilename)(filename));
    try {
        fs_1.default.writeFileSync(outputFile, data);
    }
    catch (error) {
        if (error) {
...
function displayError(error, endpoint) {
    if (error instanceof Error) {
        console.error((0, json_colorizer_1.colorize)((0, output_error_1.outputError)(error)));
    }
    else {
        console.error(`\u001B[91mError calling the ${endpoint} endpoint: ${String(error)}\u001B[0m`);
    }
}
