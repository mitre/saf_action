"use strict";
// utils/dataLoader.ts
Object.defineProperty(exports, "__esModule", { value: true });
exports.loadExecJSONs = void 0;
const tslib_1 = require("tslib");
const fs_1 = tslib_1.__importDefault(require("fs"));
const inspecjs_1 = require("inspecjs");
const logging_1 = require("../logging");
const UTF8_ENCODING = 'utf8';
/**
* The logger for this command.
 * It uses a Winston logger with the label 'view summary:'.
 * @property {ReturnType<typeof createWinstonLogger>} logger - The logger for this command. It uses a Winston logger with the label 'view summary:'.
 */
const logger = (0, logging_1.createWinstonLogger)('View Summary:');
function loadExecJSONs(files) {
    logger.verbose('In loadExecJSONs');
    const execJSONs = {};
    files.forEach((file) => {
        execJSONs[file] = (0, inspecjs_1.convertFileContextual)(fs_1.default.readFileSync(file, UTF8_ENCODING));
    });
    return execJSONs;
}
exports.loadExecJSONs = loadExecJSONs;
//# sourceMappingURL=dataLoader.js.map