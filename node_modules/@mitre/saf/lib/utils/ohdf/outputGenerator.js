...
exports.generateMarkdownTable = exports.prettyPrintColumnTitle = exports.prettyPrintRowTitle = exports.convertToMarkdown = exports.generateMarkdownTableRow = exports.generateValue = exports.extractResultSets = exports.createPrintableSummary = exports.printAndWriteOutput = exports.COLUMN_EMOJI = exports.COLUMN_ORDER = exports.ROW_ORDER = void 0;
...
exports.ROW_ORDER = ['total', 'critical', 'high', 'medium', 'low'];
exports.COLUMN_ORDER = ['passed', 'failed', 'skipped', 'no_impact', 'error'];
exports.COLUMN_EMOJI = {
 ...
 */
function printAndWriteOutput(args) {
    ...
    switch (args.format) {
        ...
        default: { // Default to 'yaml'
            outputStr = yaml_1.default.stringify(args.printableSummaries);
        }
    }
    if (args.stdout) {
    ...
    if (args.output) {
        try {
            fs_1.default.writeFileSync(args.output, outputStr);
            logger.info(`Output written to ${args.output}`);
        }
        catch (error) {
            logger.error(`Failed to write output to ${args.output}: ${error.message}`);
        }
    }
}
exports.printAndWriteOutput = printAndWriteOutput;
/**
 ...
 */
function createPrintableSummary(profileName, profileMetrics, execJSONs, complianceScores) {
...
exports.createPrintableSummary = createPrintableSummary;
/**
 ...
 */
function extractResultSets(execJSONs, profileName) {
...
exports.extractResultSets = extractResultSets;
/**
 ...
 */
function generateValue(item, column, key) {
 ...
 */
function generateMarkdownTableRow(row, item) {
    return exports.COLUMN_ORDER.map(column => generateValue(item, column, row));
}
exports.generateMarkdownTableRow = generateMarkdownTableRow;
/**
 ...
 */
function convertToMarkdown(data, titleTables) {
 ...
 */
function prettyPrintRowTitle(title) {
    return title.charAt(0).toUpperCase() + title.slice(1);
}
exports.prettyPrintRowTitle = prettyPrintRowTitle;
/**
 ...
 */
function prettyPrintColumnTitle(title) {
...
exports.prettyPrintColumnTitle = prettyPrintColumnTitle;
/**
 ...
 */
function generateMarkdownTable(item, titleTables) {
    const score = item.compliance.toString();
    const headerRow = ['Compliance: ' + score + '<br>:test_tube:', ...exports.COLUMN_ORDER.map(column => `${prettyPrintColumnTitle(column)}<br>${exports.COLUMN_EMOJI[column]}`)];
    const table = exports.ROW_ORDER.map(row => [prettyPrintRowTitle(row), ...generateMarkdownTableRow(row, item)]);
    const myTable = {
        head: headerRow,
        body: table,
    };
    const myAlignment = [markdown_table_ts_1.Align.Left, markdown_table_ts_1.Align.Center, markdown_table_ts_1.Align.Center, markdown_table_ts_1.Align.Center, markdown_table_ts_1.Align.Center, markdown_table_ts_1.Align.Center];
    ...
    return title + (0, markdown_table_ts_1.getMarkdownTable)({
        table: myTable,
