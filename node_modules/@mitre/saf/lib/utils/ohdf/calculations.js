...
function calculateComplianceScoresForExecJSONs(execJSONs) {
    ...
    for (const parsedExecJSON of Object.values(execJSONs)) {
        ...
        lodash_1.default.set(complianceScores, `["${profileName.replaceAll('"', String.raw `\"`)}"]`, existingCompliance);
    }
...
function calculateTotalCountsForSummaries(summaries) {
    ...
    for (const [profileName, profileSummaries] of Object.entries(summaries)) {
        for (const profileSummary of profileSummaries) {
            const flattened = (0, threshold_1.flattenProfileSummary)(profileSummary);
            for (const [key, value] of Object.entries(flattened)) {
                const existingValue = lodash_1.default.get(totals, `${profileName}.${key}`, 0);
                if (typeof existingValue === 'number') {
                    lodash_1.default.set(totals, `["${profileName.replaceAll('"', String.raw `\"`)}"].${key}`, existingValue + value);
                }
                else {
                    lodash_1.default.set(totals, `["${profileName.replaceAll('"', String.raw `\"`)}"].${key}`, value);
                }
            }
        }
    }
