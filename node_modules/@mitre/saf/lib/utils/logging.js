 ...
 */
function createWinstonLogger(module = 'SAF CLI', level = 'info') {
    const colorizer = winston_1.format.colorize({ colors: syslogColors });
    ...
    return (0, winston_1.createLogger)({
        ...
        }), winston_1.format.errors({ stack: true }), winston_1.format.printf((info) => {
            const prefix = colorizer.colorize('prefix', `[${info.timestamp} -> ${module}]`);
            const lvl = colorizer.colorize(info.level, info.level);
            let msg = '';
            if (info.message) {
                msg = lodash_1.default.isString(info.message) ? info.message : JSON.stringify(info.message, null, 2);
            }
            if (info.stack) {
                msg += `\n${lodash_1.default.isString(info.stack) ? info.stack : JSON.stringify(info.stack, null, 2)}`;
            }
