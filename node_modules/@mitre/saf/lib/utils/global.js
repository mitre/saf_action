...
exports.arrayedPaths = exports.knownInspecMetadataKeys = void 0;
exports.checkSuffix = checkSuffix;
exports.resolveSafeChild = resolveSafeChild;
exports.basename = basename;
exports.safeFilename = safeFilename;
exports.safeExecFileSync = safeExecFileSync;
...
exports.getJsonMetaDataExamples = getJsonMetaDataExamples;
...
function checkSuffix(input, suffix = '.json') {
 ...
 */
function resolveSafeChild(baseDir, ...parts) {
    const resolvedBase = fs_1.default.realpathSync(baseDir);
    const target = path_1.default.resolve(resolvedBase, ...parts);
    const rel = path_1.default.relative(resolvedBase, target);
    if (rel.startsWith('..') || path_1.default.isAbsolute(rel)) {
        throw new Error(`Refusing to write outside output directory: ${target} is not inside ${resolvedBase}`);
    }
    // Walk from baseDir down to the file's parent; reject any symlink segment.
    const parentDirParts = path_1.default.dirname(rel).split(path_1.default.sep).filter(Boolean);
    let cursor = resolvedBase;
    for (const segment of parentDirParts) {
        cursor = path_1.default.join(cursor, segment);
        if (fs_1.default.existsSync(cursor) && fs_1.default.lstatSync(cursor).isSymbolicLink()) {
            throw new Error(`Refusing to follow symlink under output directory: ${cursor}`);
        }
    }
    if (fs_1.default.existsSync(target) && fs_1.default.lstatSync(target).isSymbolicLink()) {
...
function basename(inputPath) {
    return path_1.default.win32.basename(inputPath.trimEnd());
}
/**
...
function safeFilename(inputPath) {
    ...
    if (filename === '' || (0, sanitize_filename_1.default)(filename) !== filename) {
...
function safeExecFileSync(command, args, options) {
    ...
    return (0, child_process_1.execFileSync)(command, args, { ...options, shell: process_1.default.platform === 'win32' });
}
/**
 ...
 */
function getInstalledPath(moduleName) {
    let installedPath;
    try {
        installedPath = (0, get_installed_path_1.getInstalledPathSync)(moduleName);
    }
    catch {
        installedPath = app_root_path_1.default.path;
    }
...
function getProfileInfo(evaluation, fileName) {
    ...
    const profile = evaluation?.profiles ? evaluation.profiles[0] : undefined;
...
function getDescription(descriptions, key) {
    return Array.isArray(descriptions)
        ? descriptions.find((description) => description.label.toLowerCase() === key)?.data
        : lodash_1.default.get(descriptions, key);
}
/**
...
function checkInput(guessOptions, desiredType, desiredFormat) {
    const detectedType = (0, hdf_converters_1.fingerprint)({ data: guessOptions.data, filename: basename(guessOptions.filename) });
    if (!(detectedType === desiredType)) {
        throw new Error(`Unable to process input file\
      \nDetected input type: ${detectedType === hdf_converters_1.INPUT_TYPES.NOT_FOUND ? 'unknown or none' : `${detectedType} - did you mean to run the ${detectedType} to HDF converter instead?`}\
 ...
 */
function extractFileFromZip(zipPath, fileName) {
    try {
 ...
 */
function getJsonMetaDataExamples(endpoint) {
    const profile = '{ '
    ...
    const assets = '"marking": "[Unclass, CUI, etc]",'
        ...
        + `"role": "The computing role, one of: [${Object.values(hdf_converters_1.Role).join(', ')}]",`
        + `"assettype": "The asset type, one of: [${Object.values(hdf_converters_1.Assettype).join(', ')}]",`
        + `"techarea": "The tech area, one of: [${Object.values(hdf_converters_1.Techarea).filter(item => item !== hdf_converters_1.Techarea.Empty).join(', ')}]",`
