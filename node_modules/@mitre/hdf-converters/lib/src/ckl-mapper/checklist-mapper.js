...
function transformImpact(vuln) {
    if (vuln.status === 'Not Applicable')
        return 0.0;
    let impact = ImpactMapping[findSeverity(vuln).toLowerCase()];
    if (isJsonString(vuln.thirdPartyTools)) {
        const hdfExistingData = JSON.parse(vuln.thirdPartyTools);
        impact = lodash_1.default.get(hdfExistingData, 'hdfSpecificData.impact', ImpactMapping[findSeverity(vuln).toLowerCase()]);
    }
    if (!impact)
        throw new Error(`Severity "${findSeverity(vuln)}" does not match low, medium, or high, please check severity for ${vuln.vulnNum}`);
    return impact;
}
...
function getHdfSpecificDataAttribute(attribute, input) {
    ...
    if (!input ||
        !isJsonString(input) ||
        !lodash_1.default.isObject((data = JSON.parse(input)).hdfSpecificData)) {
        return undefined;
    }
                            ...
                            code: {
                                transformer: (vulnerability) => {
                                    ...
                                    if (isJsonString(vulnerability.thirdPartyTools)) {
                                        return (_a = JSON.parse(vulnerability.thirdPartyTools)
                                            .hdfSpecificData) === null || _a === void 0 ? void 0 : _a.code;
                                    }
