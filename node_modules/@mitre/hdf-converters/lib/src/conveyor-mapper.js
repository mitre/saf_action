...
function createDescription(data, score, date, scannerName, endTime) {
    const desc = () => {
      ...
      heuristic_name:${_.get(data, 'heuristic.name')}`.replace('\\"', '');
        }
      ...
      classificaton:${_.get(data, 'classification')}
      depth:${_.get(data, 'depth')}
      title_text:${_.get(data, 'title_text')}`.replace('\\"', '');
        }
        else {
...
function controlMappingConveyor() {
    return {
        ...
        tags: {
            ...
            cci: global_1.DEFAULT_STATIC_CODE_ANALYSIS_CCI_TAGS
        },
        source_location: {},
        results: [
