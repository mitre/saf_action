...
function getSecurityHub() {
    ...
    const findingNistTag = (finding, { awsConfigMapping }) => {
        ...
        const matches = awsConfigMapping.data.filter((element) => {
            var _a;
            return (_a = lodash_1.default.get(finding, 'ProductFields.RelatedAWSResources:0/name')) === null || _a === void 0 ? void 0 : _a.includes(element.configRuleName);
        });
        return lodash_1.default.uniq(matches.map((rule) => rule.nistId.split('|')).flat());
    };
