...
exports.FromHDFToHTMLMapper = exports.FileExportTypes = void 0;
...
const embedded_assets_1 = require("./embedded-assets");
...
})(FileExportTypes || (exports.FileExportTypes = FileExportTypes = {}));
...
class FromHDFToHTMLMapper {
    ...
    constructor(files, exportType) {
        ...
        for (const file of files) {
            ...
            if (lodash_1.default.isString(file.data)) {
    ...
    addFiledata(file, exportType) {
        ...
        this.outputData.files.push({
            filename: file.fileName,
            toolVersion: lodash_1.default.get(file.data, 'data.version'),
            platform: lodash_1.default.get(file.data, 'data.platform.name'),
            duration: lodash_1.default.get(file.data, 'data.statistics.duration')
        });
        // Set export type
        ...
        const allResultLevels = file.data.contains.reduce((acc, profile) => {
            const matchingResults = profile.contains.filter((result) => !filteredControlsSet || filteredControlsSet.has(result.data.id));
    ...
    addResultDetails(result, resultLevels) {
        ...
        const severity = lodash_1.default.capitalize(result.root.hdf.severity);
        ...
        // Assign attributes
        return {
            ...lodash_1.default.set(result, 'hdf.segments', [].concat.apply([], resultLevels.map((resultLevel) => resultLevel.hdf.segments || []))),
            ...
            ].filter((v) => v.value).map((s) => ({ ...s, value: (0, sanitize_html_1.default)(s.value, { disallowedTagsMode: "escape" }) })),
    ...
    async toHTML() {
        ...
        const template = embedded_assets_1.html;
        this.outputData.tailwindStyles = embedded_assets_1.css;
        // Remove source map reference in TW Elements library
        this.outputData.tailwindElements = embedded_assets_1.js.replace('//# sourceMappingURL=tw-elements.umd.min.js.map', '');
