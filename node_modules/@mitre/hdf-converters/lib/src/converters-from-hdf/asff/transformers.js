...
function escapeForwardSlashes(s) {
    return lodash_1.default.isString(s)
        ? s.replace(/\//g, reverse_asff_mapper_1.TO_ASFF_TYPES_SLASH_REPLACEMENT)
        : JSON.stringify(s).replace(/\//g, reverse_asff_mapper_1.TO_ASFF_TYPES_SLASH_REPLACEMENT);
}
...
function createCode(control) {
    var _a, _b;
    const noCodeValue = (lodash_1.default.get(control, 'profileInfo.depends') || []).length > 0
        ? ''
        : JSON.stringify(lodash_1.default.omitBy(lodash_1.default.omit(control, ['results', 'profileInfo']), cleanObjectValues));
...
function createControlMetadata(control) {
    ...
    if (control.refs && control.refs.length > 0) {
        types.push(`Control/Refs/${escapeForwardSlashes(JSON.stringify(control.refs))}`);
    }
    if (control.source_location &&
        Object.keys(control.source_location).length > 0) {
        types.push(`Control/Source_Location/${escapeForwardSlashes(JSON.stringify(control.source_location))}`);
    }
    if (control.waiver_data && Object.keys(control.waiver_data).length > 0) {
        types.push(`Control/Waiver_Data/${escapeForwardSlashes(JSON.stringify(control.waiver_data))}`);
    }
    if (control.title) {
...
function createSegmentInfo(segment) {
    ...
    targets.forEach((target) => {
        if (lodash_1.default.has(segment, target) && lodash_1.default.get(segment, target) !== undefined) {
            if (lodash_1.default.get(segment, target) === '') {
                typesArr.push(`Segment/${escapeForwardSlashes(target)}/''`);
            }
            else {
                typesArr.push(`Segment/${escapeForwardSlashes(target)}/${escapeForwardSlashes(lodash_1.default.get(segment, target))}`);
            }
        }
    });
    return typesArr;
...
function createTagInfo(control) {
    ...
    for (const tag in control.tags) {
        typesArr.push(`Tags/${escapeForwardSlashes(tag)}/${escapeForwardSlashes(JSON.stringify(control.tags[tag]))}`);
    }
    return typesArr;
}
    ...
    if (Array.isArray(control.descriptions)) {
        (_a = control.descriptions) === null || _a === void 0 ? void 0 : _a.forEach((description) => {
            if (description.data && cleanText(description.data)) {
                typesArr.push(`Descriptions/${escapeForwardSlashes(description.label)}/${escapeForwardSlashes(cleanText(description.data))}`);
            }
        });
    }
    else {
        Object.entries(control.descriptions || {}).forEach(([key, value]) => {
            if (value && cleanText(value)) {
                typesArr.push(`Descriptions/${escapeForwardSlashes(key)}/${escapeForwardSlashes(cleanText(value))}`);
            }
        });
    }
    return typesArr;
