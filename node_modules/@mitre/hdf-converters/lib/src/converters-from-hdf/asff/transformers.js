...
function createProfileInfoFindingFields(hdf, options) {
    ...
    executionTargets.forEach((target) => {
        ...
        if (lodash_1.default.isString(value) && value.trim()) {
    ...
    const passthrough = lodash_1.default.get(hdf, 'passthrough');
    if (lodash_1.default.isString(passthrough) && passthrough.trim()) {
    ...
    targets.forEach((target) => {
        if (lodash_1.default.has(segment, target) && lodash_1.default.get(segment, target) !== undefined) {
            if (lodash_1.default.get(segment, target) === '') {
                ...
                typesArr.push(`Segment/${(0, reverse_asff_mapper_1.escapeForwardSlashes)(target)}/${(0, reverse_asff_mapper_1.escapeForwardSlashes)(lodash_1.default.get(segment, target))}`);
            }
        }
    });
    return typesArr;
...
function setupFindingType(control, context) {
    ...
    typesArr.push(...lodash_1.default.compact([desc, code, codeDesc]));
    return typesArr;
}
