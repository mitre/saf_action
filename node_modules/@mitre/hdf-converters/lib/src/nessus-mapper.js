...
function getTitle(item) {
    if (lodash_1.default.has(item, COMPLIANCE_CHECK_NAME)) {
        return lodash_1.default.get(item, COMPLIANCE_CHECK_NAME);
    }
    else {
...
function getDesc(item) {
    if (lodash_1.default.has(item, COMPLIANCE_INFO)) {
        return (0, base_converter_1.parseHtml)(lodash_1.default.get(item, COMPLIANCE_INFO));
    }
    else {
...
function getCheck(item) {
    if (lodash_1.default.has(item, COMPLIANCE_SOLUTION)) {
        return (0, base_converter_1.parseHtml)(lodash_1.default.get(item, COMPLIANCE_SOLUTION));
    }
    else {
        return '';
    }
...
function getFix(item) {
    const fix = lodash_1.default.get(item, 'solution');
    if (fix && fix !== 'n/a') {
        return fix;
    }
    return '';
...
function getStatus(item) {
    const result = lodash_1.default.get(item, COMPLIANCE_RESULT, '');
    switch (result) {
        case 'PASSED':
                ...
                transformer: (data) => {
                    return {
                        auxiliary_data: [
                            {
                                name: 'Nessus',
                                data: lodash_1.default.omit(data, ['name', 'ReportItem'])
                            }
                        ],
